...
class SimulatedHost : public TimerListHost {
 public:
  SimulatedHost(FuzzingEventEngine* engine) : engine_(engine) {};
...
TEST(TimerTrainTest, AddCancelAndExtend) {
  ...
  TimerTrain timer_train(
      std::move(host),
      TimerTrain::Options{
          .period = kTrainPeriod, .num_shards = 5, .event_engine = engine});
...
void BM_TimerTrainExtend(benchmark::State& state) {
  ...
  std::shared_ptr<EventEngine> engine = GetDefaultEventEngine();
  constexpr int kNumTimers = 1000;
  TimerTrain timer_train(TimerTrain::Options{
      .period = kTrainPeriod, .num_shards = 32, .event_engine = engine});
  EventEngine::TaskHandle handle[kNumTimers];
  for (int i = 0; i < kNumTimers; ++i) {
